...
int search  (const char *string, size_t strlen, int32_t *ip4_address) {
  ...
  //printf("Obtained lock in search\n");
  struct trie_node *found;

  // Skip strings of length 0
  if (strlen == 0) {
    pthread_rwlock_unlock(&lock);
     // printf("Released lock in search\n");
    return 0;
  }
...
int insert (const char *string, size_t strlen, int32_t ip4_address) {
  ...
  //printf("Obtained lock in insert\n");
  // Skip strings of length 0
  if (strlen == 0) {
    pthread_rwlock_unlock(&lock);
  ...
  //check_max_nodes();//call drop_one_node() until we are under the limit
  pthread_rwlock_unlock(&lock);
  //printf("Released lock in insert\n");
  return rv;
}
...
int delete  (const char *string, size_t strlen) {
  pthread_rwlock_wrlock(&lock);
 // printf("Obtained lock in delete.\n");

  // Skip strings of length 0
  if (strlen == 0) {
    pthread_rwlock_unlock(&lock);
    //printf("Released lock in delete.\n");
    return 0;
  }
  assert(strlen < 64);
  //printf("Released lock in delete.\n");
  int rv = _delete(root, string, strlen);
  pthread_rwlock_unlock(&lock);
  return (NULL != rv);
}
