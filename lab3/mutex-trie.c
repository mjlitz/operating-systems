...
static pthread_cond_t maxnodes;


void init(int numthreads) {
    pthread_mutex_init(&lock,NULL);
    pthread_cond_init(&maxnodes, NULL);
}

struct trie_node * new_leaf (const char *string, size_t strlen, int32_t ip4_address) {
  struct trie_node *new_node = malloc(sizeof(struct trie_node));
  if (separate_delete_thread)
    pthread_cond_signal(&maxnodes);
  node_count++;
  if (!new_node) {
...
int _insert (const char *string, size_t strlen, int32_t ip4_address,
  ...
  if (cmp == 0) {
    ...
    if (overlap) {
      ...
      struct trie_node *new_node = new_leaf (&string[offset], keylen2, -1);
...
int insert (const char *string, size_t strlen, int32_t ip4_address) {
  ...
  if (strlen == 0) {
    pthread_mutex_unlock(&lock);
     // printf("Released lock in insert\n");
    return 0;
  }
...
int remove_node_references (struct trie_node *current, struct trie_node *target) {
  if (current->next == target)
    current->next = target->next;
  if (current->next)
    remove_node_references(current->next,target);
  if (current->children == target)
    current->children = current->children->next;
  if (current->children)
    remove_node_references(current->children,target);
  return 0;
}
...
int drop_one_node  () {
  ...
  while (node->next || node->children){
    while(node->next)
      node = node->next;
    while (node->children) {
      //add key and len to child, recombining the key to its original form from common nodes.
      strlen+= node->strlen;
      strncat(node->key,str,strlen);
      node = node->children;
    }
  }
  //add key and len of node itself to be deleted.
  strlen+= node->strlen;
  strncat(str,node->key,strlen);
  printf("Dropping node %p.\n", node);
  delete(str,strlen);
  return 0;
}
...
void check_max_nodes  () {
  if (separate_delete_thread){
    ...
    pthread_cond_wait(&maxnodes, &lock);
